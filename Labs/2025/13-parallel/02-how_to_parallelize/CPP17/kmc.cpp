...
int main() {
...
#if (__cplusplus == 202302L)
  auto counter=std::views::iota(0, Np);
  std::cout << "using c++23 features\n";
#else
  range counter(0, Np);
  std::cout << "not using c++23 features\n";
#endif
  ...
  for (int it = 0; it<Nt; ++it){    
    ...
    std::for_each (counter.begin (), counter.end (), state);
    //oneapi::tbb::parallel_for_each(counter.begin (), counter.end (), state);
