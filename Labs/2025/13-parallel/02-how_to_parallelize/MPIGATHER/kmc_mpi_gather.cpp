...
int main(int argc, char* argv[]) {
    ...
    std::vector<int> rank_local(local_Np);
    rank_local.assign(local_Np, rank);
    ...
    for (int it = 0; it < Nt; ++it) {
        ...
        MPI_Gatherv(y_local.data(), local_Np, MPI_DOUBLE,
                    ...
                    0, MPI_COMM_WORLD);
	MPI_Gatherv(rank_local.data(), local_Np, MPI_INT,
                    rank_all.data(), counts.data(), displs.data(), MPI_INT,
                    0, MPI_COMM_WORLD);
	    ...
	    out << "x" << "," << "y"<< "," << "rank" << "\n";
            for (int i = 0; i < Np; ++i)
	      out << x_all[i] << "," << y_all[i] << "," << rank_all[i] << "\n";
            out.close();
        }
