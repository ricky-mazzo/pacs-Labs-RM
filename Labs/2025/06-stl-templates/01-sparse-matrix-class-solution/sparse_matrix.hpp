...
public :
  ...
  inline const size_t rows () const {return size ();} // Rmk: compare with templates solutions
  ...
  void 
  operator += (sparse_matrix &rhs);
  
  friend std::vector <double> 
  operator*(sparse_matrix & M, const std::vector<double> &x);
  
  friend std::ostream & 
  operator << (std::ostream & os, sparse_matrix &M); //return os;
